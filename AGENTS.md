...
Эти правила не привязаны к одному агенту. Они рассчитаны на работу через
Copilot в VS Code, Codex, ChatGPT/OpenAI и другие агентные среды, если у них
есть доступ к репозиторию, терминалу или API.
...
6. **Data-only changes** через CLI не требуют push — данные синкаются
   через PostgreSQL мгновенно.
7. **После изменения кода** (UI, components, API routes) обязательно:
   `npm run alpha -- deploy --message "описание"` — это type-check + commit + push.
8. Независимо от того, работаешь ли ты из VS Code, Codex или ChatGPT, используй
   один и тот же путь записи данных: CLI/API, а не ручное редактирование
   хранилищ.
